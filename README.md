...
A value written as an XML entity reference is expanded and treated like any
other, so it is read, compared and cleaned the same way. An entity that points
outside the file is never fetched: it stays undefined and the file is rejected,
rather than its target being pulled into the output.
